...
The system SHALL provide a page with a left sidebar containing a "Create New Chat" button, a search field, and a scrollable list of chats sorted by creation date (newest first), and a right main area displaying messages in a scrollable area with proper styling for user and AI messages, where messages have a minimum width even for short text.

#### Scenario: User views chat messages
- **WHEN** user navigates to the chat page
- **THEN** messages are displayed in chronological order with distinct styling for user and AI messages in the right area, with minimum width for short messages
- **AND** a sidebar on the left shows a "Create New Chat" button, search field, and scrollable list of chats sorted by creation date

### Requirement: Implement message input functionality
The system SHALL provide a multi-line input field with increased initial height where users can type and send messages to the AI, with a styled scrollbar that hides the track and shows only the thumb.
...
The system SHALL use consistent, modern styling with proper spacing, colors, responsive design, full height layout, minimal margins, and custom scrollbar styling.

#### Scenario: Chat interface is visually appealing
- **WHEN** user views the chat page
- **THEN** the interface uses modern design principles with good typography, spacing, full height layout, and custom scrollbar styling
